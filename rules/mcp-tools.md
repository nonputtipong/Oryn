...
MCP server tools are available as **direct tool calls** -- just like `Read`, `Grep`, or `Bash`. They are NOT CLI commands. Call them directly as tools.
...
```bash
Bash: <mcp cli command>  # WRONG -- not a CLI command
```
...
AI Agents default to sequential MCP tool calls even when tools are independent. For multi-tool workflows, explicitly mark independent calls for parallel execution:
